...
--(변경 명령어 : ALTER TABLE 기존테이블명 RENAME 새테이블명;) !!!''사용하지 않기
...
-- 24. sinsang테이블에 content 필드를 text 타입으로 추가하시오. //add column : 필드 추가
...
-- 26. sinsang테이블에 content 필드를 soge필드로 변경하시오. 타입은 똑같은 text타입이다. //change column : 이름을 바꿈
alter table sinsang change column content soge text;


-- 27. sinsang테이블의 soge필드의 타입을 가변길이 100Byte로 변경하시오.(null값은 허용한다) //modify column:속성을 바꿈
alter table sinsang modify column soge varchar(100);


-- 28. sinsang테이블에 content 필드를 삭제하시오.(실제로 삭제하지는 말고 명령어만 사용하시오. 만약 삭제하였다면 새롭게 content필드를 가변길이 100Byte로 추가하시오) //drop column
...
-- 30. sinsang테이블의 sex필드명을 gender필드명으로 변경하시오.(고정길이문자(2), 기본값(여자)) //change column : 이름을 바꿈
...
-- 27. soge 필드의 추가로 입력된 내용 그대로 출력 오류 - default 추가
select * from sinsang;
...
-- 56. '서울'지역 사용자는 비밀번호를 '0000'으로, 포인트는 300점을 추가로 지급하시오. //, 사용
update sinsang set pwd = '0000', point = point + 300 where address = '서울';
select * from sinsang;

-- 57. 포인트가 없고, 남자이면서 1985년 이전에 입사한 회원을 삭제처리하시오. //and 사용
